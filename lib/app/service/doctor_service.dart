...
// Get doctor and all its property
  ...
  //     var doctorRef = await FirebaseFirestore.instance
  //         .collection('Doctors')
  //         //.where('doctorName', isGreaterThanOrEqualTo: doctorName.toUpperCase())
  //         .where('doctorName',
  ...
  //           )
  //     //     .where('doctorName',
  //     //     isGreaterThanOrEqualTo: doctorName.toUpperCase(),
  //     //
  //     //     isLessThan: doctorName.substring(0, doctorName.length - 1) +
  //     //         String.fromCharCode(
  //     //             doctorName.codeUnitAt(doctorName.length - 1) + 1)
  ...
  Future<List<Doctor>> searchDoctor(String doctorName) async {
    try {
      ...
      var doctorRef = await FirebaseFirestore.instance
          .collection('Doctors')
           //.where('doctorName', isGreaterThanOrEqualTo: doctorName.toUpperCase())
          // .where('doctorName', isLessThanOrEqualTo: doctorName.toLowerCase()+'\uf8ff')
          // //.where('doctorName',arrayContains: doctorName)

          .where('searchName',
                  isGreaterThanOrEqualTo: doctorName,
                  isLessThan: doctorName.substring(0, doctorName.length - 1)
                      + String.fromCharCode(doctorName.codeUnitAt(doctorName.length - 1) + 1)
                    )
          .get();
